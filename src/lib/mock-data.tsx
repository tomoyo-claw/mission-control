...
export interface Event {
  ...
  recurring?: string; // cron expression or interval description
  createdBy: string;
  createdAt: number;
  updatedAt: number;
}
...
const mockEvents: Event[] = [
  // Cron jobs
  {
    ...
    description: 'HEARTBEAT.md に基づく定期チェック（メール、カレンダー、メンション）',
    startDate: Date.now() - 30 * 60 * 1000,
    endDate: Date.now() - 29 * 60 * 1000,
    category: 'cron',
    color: '#8B5CF6',
    assignee: 'ai',
    status: 'completed',
  ...
  },
  {
    _id: 'cron-2',
    title: 'Heartbeat チェック',
    description: '次回のハートビートポーリング',
    startDate: Date.now() + 30 * 60 * 1000,
    endDate: Date.now() + 31 * 60 * 1000,
    category: 'cron',
    color: '#8B5CF6',
    assignee: 'ai',
    status: 'scheduled',
  ...
  },
  {
    ...
    description: 'PCのChromeにOpenClaw Browser Relay拡張をインストール・設定',
  ...
  {
    _id: 'task-3',
    title: 'X (@eternum_zak) ログイン & リサーチ開始',
    description: 'ブラウザリレー経由でXにログイン。DeFi/AI関連のリサーチ開始',
    startDate: Date.now() + 2 * 24 * 60 * 60 * 1000,
    endDate: Date.now() + 2 * 24 * 60 * 60 * 1000 + 2 * 60 * 60 * 1000,
    category: 'task',
    color: '#3B82F6',
    assignee: 'ai',
    status: 'scheduled',
  ...
  {
    _id: 'milestone-1',
    title: 'Mission Control v1 完成',
    description: '全6画面の実装完了',
  ...
  },
  {
    _id: 'done-4',
    title: 'メモリ画面改修',
    description: '検索 + ドキュメントビューア',
    startDate: Date.now() - 1 * 60 * 60 * 1000,
  ...
  // Reminders
  {
    _id: 'reminder-1',
    title: 'Vercelトークン再設定',
    description: 'vercel loginでトークンを再取得する',
    startDate: Date.now() + 5 * 60 * 60 * 1000,
    endDate: Date.now() + 5 * 60 * 60 * 1000,
    category: 'reminder',
    color: '#EC4899',
    assignee: 'zak',
    status: 'scheduled',
